...
import { validationForPassword, validationForEmail } from "@/lib/seculity/validation";
...
export default function SignIn({
    ...
    userType: UserType
     ...
     const signInWithUserType = signIn.bind(null, userType);
    ...
    const handleAction = (formData:FormData) => {
        ...
        //email,passwordのvalidation結果を反映
                ...
                {!state.success
                    ?(<> 
                        ...
                        >
                            ...
                            <Link 
                                href="/auth/resetPass" 
                                className="inline-block align-middle mt-3 font-bold text-sm text-blue-400 hover:text-blue-600"
                            >
                                passwordをお忘れですか？
                            </Link>
                        </form>
                    </> ):(
                        <MailAuth email={signForm.email[0]} typeValue={'SignIn'} userType={userType}/>
                    )
                }
            </div>
        </div>
