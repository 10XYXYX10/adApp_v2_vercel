'use client'
//「src/components/ad/preview/YouTubeShortAdComponent.tsx」
import React, { useState, useEffect } from 'react';

const YouTubeShortAdComponent = () => {
    ...
    if (phase === 'youtube-ad' && isYoutubePlaying) {
      if (adTimer > 0) {
        const timer = setTimeout(() => {
          ...
          // 15秒経過後にスキップボタン表示
          if (adTimer === 106) { // 120-106=14, つまり15秒後
          ...
          <>
            ...
            {showSkipButton ? (
              ...
              <div className="absolute bottom-4 right-4 bg-gray-800 bg-opacity-90 text-white px-4 py-2 rounded text-sm font-medium">
                広告をスキップまであと {Math.max(0, adTimer - 105)}秒
              </div>
            )}
      ...
      <div className="bg-gray-900 p-4 text-white">
        ...
        <div className="text-xs text-gray-300 space-y-1">
          ...
          <div className="flex items-center space-x-2">
            ...
            <span>2. YouTube動画再生!15秒強制視聴！(*longの場合は30秒)</span>
          </div>
          <div className="flex items-center space-x-2">
...
export default YouTubeShortAdComponent;